...
class MVATools(object):
    ...
    def plot_mcr_factors(self, comp_ids=None, calibrate=True,
                         same_window=True, comp_label=None,
                         per_row=3, title=None):
        """Plot factors from MCR. In case of 1D
        signal axis, each factors line can be toggled on and off by clicking
        on their corresponding line in the legend.
        ...
        See Also
        --------
        plot_mcr_loadings, plot_mcr_results.
        ...
        if same_window is None:
            same_window = True
        factors = self.learning_results.mcr_factors
        title = _change_API_comp_label(title, comp_label)
        if title is None:
            title = self._get_plot_title('MCR factors of', same_window)
    ...
    def plot_mcr_loadings(self, comp_ids=None, calibrate=True,
                          same_window=True, comp_label=None,
                          with_factors=False, cmap=plt.cm.gray,
                          no_nans=False, per_row=3, axes_decor='all',
                          title=None):
        """Plot loadings from MCR results. In case of 1D
        navigation axis, each loading line can be toggled on and off by
        clicking on their corresponding line in the legend.
        ...
        See Also
        --------
        plot_mcr_factors, plot_mcr_results.
        ...
        title = _change_API_comp_label(title, comp_label)
        if title is None:
            title = self._get_plot_title('MCR loadings of',
                                         same_window)
        loadings = self.learning_results.mcr_loadings.T
        if with_factors:
            factors = self.learning_results.mcr_factors
        else:
            factors = None
    ...
    def get_mcr_loadings(self):
        """Return the MCR loadings as a Signal.
        ...
        """
        signal = self._get_loadings(
            self.learning_results.mcr_loadings)
        signal.axes_manager[0].name = "MCR component index"
        signal.metadata.General.title = ("MCR loadings of " +
                                         self.metadata.General.title)
        return signal

    def get_mcr_factors(self):
        """Return the MCR factors as a Signal.
        ...
        """
        signal = self._get_factors(self.learning_results.mcr_factors)
        signal.axes_manager[0].name = "MCR component index"
        signal.metadata.General.title = ("MCR factors of " +
                                         self.metadata.General.title)
        return signal

    def plot_mcr_results(self,
                         factors_navigator="smart_auto",
                         loadings_navigator="smart_auto",
                         factors_dim=2,
                         loadings_dim=2,):
        """Plot the MCR factors and loadings.

        Unlike `plot_mcr_factors` and `plot_mcr_loadings`, this method displays
        one component at a time. Therefore it provides a more compact
        visualization than then other two methods.  The loadings and factors
        are displayed in different windows and each has its own
        navigator/sliders to navigate them if they are multidimensional. The
        ...
        factors_dim, loadings_dim: int
            Currently HyperSpy cannot plot signals of dimension higher than
            two. Therefore, to visualize the MCR results when the
            factors or the loadings have signal dimension greater than 2
            we can view the data as spectra(images) by setting this parameter
            to 1(2). (Default 2)
        ...
        See Also
        --------
        plot_mcr_factors, plot_mcr_loadings.

        """
        factors = self.get_mcr_factors()
        loadings = self.get_mcr_loadings()
        _plot_x_results(factors=factors, loadings=loadings,
                        factors_navigator=factors_navigator,
                        loadings_navigator=loadings_navigator,
                        factors_dim=factors_dim,
